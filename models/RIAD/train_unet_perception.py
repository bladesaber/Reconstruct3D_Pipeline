...
def main():
    ...
    while True:
        ...
        for i, data_batch in enumerate(dataloader):
            ...
            batch_imgs = data_batch['img']
            batch_disjoint_masks = data_batch['disjoint_masks']
            batch_disjoint_imgs = data_batch['disjoint_imgs']
            batch_masks = data_batch['mask']
            batch_l1_masks = data_batch['layer1_mask']
            batch_l2_masks = data_batch['layer2_mask']
            batch_l3_masks = data_batch['layer3_mask']

            if device == 'cuda':
                batch_imgs = batch_imgs.to(torch.device('cuda:0'))
                batch_disjoint_masks = batch_disjoint_masks.to(torch.device('cuda:0'))
                batch_disjoint_imgs = batch_disjoint_imgs.to(torch.device('cuda:0'))
                batch_masks = batch_masks.to(torch.device('cuda:0'))
                batch_l1_masks = batch_l1_masks.to(torch.device('cuda:0'))
                batch_l2_masks = batch_l2_masks.to(torch.device('cuda:0'))
                batch_l3_masks = batch_l3_masks.to(torch.device('cuda:0'))

            loss_dict = network.train_step(
                imgs=batch_imgs,
                disjoint_masks=batch_disjoint_masks,
                disjoint_imgs=batch_disjoint_imgs,
                obj_masks=batch_masks,
                layer1_mask=batch_l1_masks,
                layer2_mask=batch_l2_masks,
                layer3_mask=batch_l3_masks
            )
